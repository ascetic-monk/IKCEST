...
import os
sys.path.append("../")
from worklstm_ext.dataset_lstm_ext import InfectDataset, data_split
...
class WeightedGenerate( ):
    ...
    def generate_mode(self, x, y):
        ...
        font_x = torch.mean(x[:, :5, :], axis=1)
        behi_x = torch.mean(x[:, -5:, :], axis=1)
        balan = (font_x > self.threshold[1]) * (torch.abs((font_x - behi_x)/font_x) < 0.5)
        decrease = (total_x>total_y)
        decrease_to_down = decrease * (total_y<self.threshold[1])
        return (40-12)*decrease_to_down+(12-1)*decrease+10*balan+1
...
class Sequence(nn.Module):
    def __init__(self, use_cuda, in_feat=2, out_feat=1, hid_feat=64):
        ...
        self.lstm1 = nn.LSTMCell(in_feat, hid_feat)
        self.lstm2 = nn.LSTMCell(hid_feat, hid_feat)
        self.linear = nn.Linear(hid_feat, in_feat)
...
def train(model, criterion, optimizer, lr_schedual, dataloader, args):
    ...
    for epo in range(args.epochs):
        ...
        for idx, data in enumerate(train_loader):
            ...
            loss = criterion(y, data[:, args.n_his:args.n_his+args.n_pred, :1],
...
def inference(model, dataloader, args, future_days=30):
    ...
    for x_batch in dataloader:
        ...
        for j in range(future_days):
            ...
            if args.ylog:
                test_seq[:, args.n_his - 1, :1] = pred[:, 0, :]
            else:
                test_seq[:, args.n_his - 1, :1] = pred[:, 0, :]
